# Memora — Intelligent Photo Archive & Semantic Search System / 智能照片档案与语义搜索系统 / スマート写真アーカイブと意味検索システム

[English Version](#english-version) | [中文版 (Chinese)](#chinese-version) | [日本語版 (Japanese)](#japanese-version)
...
Memora is a **lightweight, fully self-hosted** intelligent photo archive management and semantic search system designed for individuals, families, and small creative studios. Easily deployable on a personal computer or NAS, it transforms your massive photo library into a smart knowledge base searchable using natural language.

## 🌟 Practical Utility

In daily use, Memora addresses common pain points of traditional photo management tools to deliver a smooth and practical experience:
1. **Multi-Source Library Management**: Connect and monitor multiple NAS shared folders or local directories, consolidating photos scattered across different devices.
2. **Multi-threaded Concurrent Scanning**: Optimized for network storage (like NAS over SMB), Memora uses thread-pool-based parallel `scandir` traversal to scan directories. It supports both **Incremental Scan (new files only)** and **Full Scan (updates metadata, deletes removed files)**.
3. **Fully Offline AI Descriptions & Tagging**: Connects to a local LM Studio instance (running Vision Language Models like Gemma-4-26b-a4b) to analyze photo content, perform Optical Character Recognition (OCR) on interface text, and auto-generate tags without requiring an internet connection.
4. **Asynchronous Processing Daemons**: Features independent background threads (analysis daemon, thumbnail daemon, scan scheduler) to handle compute-heavy operations. Model inference and thumbnail rendering run in the background, keeping the web interface lag-free.
5. **Modern Timeline & Adaptive UI**: Provides a responsive infinite-scroll photo grid, folder sidebars, timeline filtering, and seamless light/dark theme switching.

---

## 🚀 Key Innovations

### 1. Offline Hybrid Search System
Memora merges **high-dimensional vector similarity retrieval** with **traditional keyword matching**:
* **Vector Semantic Recall**: Uses a 768-dimensional bilingual embedding model to understand abstract search prompts like "warm atmosphere", "technology vibes", or "lonely back view". It retrieves relevant images based on meaning and mood even if the descriptions don't contain those exact words.
* **Keyword Boosting**: If the search query matches text properties in the database (such as filenames, tags, OCR text, or descriptions), the system automatically adds a significant score bonus (`Score += 1.5`).
* **The Result**: Solves the inaccuracy of purely semantic searches for exact filenames, and the rigidity of keyword searches for synonyms or themes. Exact matches rank at the top, followed by semantically related results.

### 2. 100% Privacy & Zero Cloud Dependency
All AI tasks are executed locally:
* **Locally Deployed Embedding Model**: The embedding model is packaged and deployed directly within the project folder. Vector calculation is fully functional in completely offline environments.
* **Local VLM Inference**: Image description generation relies on a local LM Studio API, ensuring your photos never leave your private network.

### 3. Network Storage Optimization (NAS-focused SMB Walk)
Includes a custom `_walk_parallel` algorithm. During directory walks over network-mounted shares, multiple subdirectory paths are recursively pushed to Python's `ThreadPoolExecutor`, reducing waiting times caused by network latency.

### 4. Lazy Loading & Thread-Safe Design
Models are loaded on demand. No deep learning memory overhead is incurred at startup or while simply browsing. The embedding model initializes only when the first query is received or when background analysis runs, remaining fully thread-safe under parallel Flask requests.
...
| :--- | :--- | :--- |
| **Backend** | **Flask (Python 3.7+)** | Lightweight, robust, and easily integrated with Python AI pipelines |
| **Database** | **SQLite (PRAGMA WAL + Row)** | Single-file database with Write-Ahead Logging (WAL) for concurrent read/write performance |
| **Embeddings** | **SentenceTransformers** | Uses `shibing624/text2vec-base-chinese` deployed locally in the project directory |
| **Vector Math** | **NumPy (float32)** | Accelerates dot-product, norm, and cosine similarity calculations (query latency < 50ms) |
| **Image Preprocessing** | **Pillow + rawpy + pillow_heif** | Standard images, HEIC/HEIF (Apple format), and ARW (Sony RAW format) support |
| **Frontend** | **Tailwind CSS + Remix Icon** | Responsive grids, glassmorphism card styles, and animated theme transitions |
| **Localization** | **i18n** | Full translation support for English, Chinese, and Japanese |

---

## 📂 Project Directory Structure

* [web_app/app.py](file:///C:/Users/团子潘/Desktop/nf/web_app/app.py) - Flask application entry point. Handles routes, hybrid search execution, session authentication, and background scheduler setup.
* [web_app/embedding_helper.py](file:///C:/Users/团子潘/Desktop/nf/web_app/embedding_helper.py) - Local embedding manager. Handles lazy loading, vector calculation, and database persistence.
* [web_app/analysis_daemon.py](file:///C:/Users/团子潘/Desktop/nf/web_app/analysis_daemon.py) - Background daemon that periodically runs image VLM analysis and triggers vector generation.
* [web_app/scanner.py](file:///C:/Users/团子潘/Desktop/nf/web_app/scanner.py) - High-concurrency SMB/NAS file scanner.
* [web_app/thumb_daemon.py](file:///C:/Users/团子潘/Desktop/nf/web_app/thumb_daemon.py) - Background thumbnail generator.
* [analyze_images_sqlite.py](file:///C:/Users/团子潘/Desktop/nf/analyze_images_sqlite.py) - Command-line batch scanner and analyzer, integrated with vector pipeline.
* [generate_vectors_backfill.py](file:///C:/Users/团子潘/Desktop/nf/generate_vectors_backfill.py) - Batch script to generate and backfill embeddings for pre-existing analyzed records.
...
If your database already contains analyzed image records, run this script to generate embeddings. This will also download and save the model locally under the `text2vec-base-chinese` directory:
```bash
...
1. Default login credentials: Username: `admin`, Password: `admin@123` (password change is required upon first login).
2. Go to `/admin` to add and activate your media sources (NAS photo root paths).
3. Set your local LM Studio API endpoint and model name in settings to start background analysis.
...
| **国際化** | **i18n** | 日本語、中国語、英語 of 3言語をワンクリックで切り替え可能 |
...
* [web_app/analysis_daemon.py](file:///C:/Users/团子潘/Desktop/nf/web_app/analysis_daemon.py) - バックグラウンド画像AI分析デーモン。ローカルVLMを定時呼び出しし、ベクトル生成を連动。
